...
void sonicSetVolume(float volume) { sonicStream.volume = volume; }
...
static int addShortSamplesToInputBuffer(short *samples, int numSamples) {
  if (numSamples == 0) {
    return 1;
  }
  memcpy(sonicStream.inputBuffer + sonicStream.numInputSamples, samples,
...
static void removeInputSamples(int position) {
  ...
  if (remainingSamples > 0) {
    memmove(sonicStream.inputBuffer, sonicStream.inputBuffer + position,
...
static void copyToOutput(short *samples, int numSamples) {
  memcpy(sonicStream.outputBuffer + sonicStream.numOutputSamples, samples,
         numSamples * sizeof(short));
...
void sonicFlushStream(void) {
  ...
  int expectedOutputSamples =
      sonicStream.numOutputSamples + (int)((remainingSamples / speed) + 0.5f);
...
int sonicSamplesAvailable(void) { return sonicStream.numOutputSamples; }
...
static int findPitchPeriod(short *samples, int preferNewPeriod) {
  ...
  if (SONIC_SKIP == 1) {
    ...
    period = findPitchPeriodInRange(sonicStream.downSampleBuffer,
                                    minPeriod / SONIC_SKIP,
...
static void overlapAdd(int numSamples, short *out, short *rampDown,
                       short *rampUp) {
  short *o;
  short *u;
  short *d;
...
static int skipPitchPeriod(short *samples, float speed, int period) {
  ...
  overlapAdd(newSamples,
             sonicStream.outputBuffer + sonicStream.numOutputSamples, samples,
             samples + period);
...
static void processStreamInput(void) {
  ...
  if (sonicStream.volume != 1.0f) {
    ...
    scaleSamples(sonicStream.outputBuffer + originalNumOutputSamples,
                 (sonicStream.numOutputSamples - originalNumOutputSamples),
                 sonicStream.volume);
  }
}
